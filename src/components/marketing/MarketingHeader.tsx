...
export function MarketingHeader({
  ...
  useEffect(() => {
    ...
    document.addEventListener("touchmove", fromScroll, { passive: true });
    window.visualViewport?.addEventListener("scroll", fromScroll, {
      passive: true,
    });
    ...
    const observer = sentinel
      ? new IntersectionObserver(
          ([entry]) => {
            sync(!entry.isIntersecting || readScrolled());
          },
          { threshold: 0 },
        )
      : null;
    ...
    return () => {
      ...
      document.removeEventListener("touchmove", fromScroll);
      window.visualViewport?.removeEventListener("scroll", fromScroll);
      observer?.disconnect();
    };
  }, []);

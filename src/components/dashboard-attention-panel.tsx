      ...
      <CardHeader className="flex flex-row flex-wrap items-start justify-between gap-4 space-y-0 pb-3">
        <div className="space-y-1.5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <CardTitle className="text-lg text-[var(--primary)]">{title}</CardTitle>
          </div>
      ...
      <CardContent className="pb-4">
        {rows.length === 0 ? (
          ...
          <ul className="space-y-2.5">
            {rows.map(({ item, plan }) => {
              const urgent =
                mode === "purchase" ? plan.purchaseIsOverdue : plan.searchIsOverdue;
              const actionDate =
                mode === "purchase" ? plan.purchaseStartsOn : plan.searchStartsOn;
              const actionTooltip =
                mode === "purchase" ? plan.tooltips.purchase : plan.tooltips.search;
              const actionLabel = mode === "purchase" ? "Comprar em" : "Buscar em";
              ...
              return (
                <li
                  ...
                  className={cn(
                    ...
                    urgent ? "border-rose-200 bg-rose-50/40" : "border-[var(--border)]",
                  )}
                >
                    ...
                    <div className="min-w-0 flex-1 space-y-1.5">
                      <div className="flex flex-wrap items-start justify-between gap-1.5">
                        ...
                        <div className="flex shrink-0 items-center gap-1.5">
                          ...
                          {urgent ? (
                            <Badge variant="overdue" className="h-6 px-2 text-[11px]">
                              Atrasado
                            </Badge>
                          ) : (
                            <Badge variant="secondary" className="h-6 px-2 text-[11px]">
                        ...
                        <MetricWithHint content={actionTooltip}>
                          <span>
                            ...
                            <span
                              className={cn(
                                "font-semibold",
                                urgent ? "text-rose-900" : "text-[var(--foreground)]",
                              )}
                            >
                              {actionDate ?? "—"}
                            </span>
...
export function DashboardAttentionPanel({
  ...
  const rows = items
    .map((item) => {
      ...
      return { item, fullPlan, purchasePlan, warehouseStock };
    });

  const fullRows = rows
    .filter(({ fullPlan }) => fullPlan.needsSchedulingAttention)
    .sort((a, b) => {
      if (a.fullPlan.searchIsOverdue !== b.fullPlan.searchIsOverdue) {
        return a.fullPlan.searchIsOverdue ? -1 : 1;
      }
      return (a.fullPlan.searchStartsAtMs ?? 0) - (b.fullPlan.searchStartsAtMs ?? 0);
    })
    .map(({ item, fullPlan }) => ({ item, plan: fullPlan }));

  const purchaseRows = rows
    .filter(({ purchasePlan }) => purchasePlan.needsPurchaseAttention)
    .sort((a, b) => {
      if (a.purchasePlan.purchaseIsOverdue !== b.purchasePlan.purchaseIsOverdue) {
